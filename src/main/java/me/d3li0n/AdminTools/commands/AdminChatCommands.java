				...
				if(sender.hasPermission("admintools.slowchat")) 
				{
					try 
					{
						if(Integer.parseInt(args[0]) > 0 && Integer.parseInt(args[0]) <= 50) 
						{
							...
							for (Player p : Bukkit.getOnlinePlayers())  
								if(p.isOnline()) p.sendMessage(ChatColor.RED + (String) (util.getLangConfig().getString("messages.commands.slowchat.disabled")).replace("%player%", sender.getName()));
					...
					catch (Exception e) 
					{
						if(!Main.CHAT_SLOW_STATE) 
						{
							Main.CHAT_SLOW_STATE = !Main.CHAT_SLOW_STATE;
							for (Player p : Bukkit.getOnlinePlayers())  
								if(p.isOnline()) p.sendMessage(ChatColor.GREEN + (String) (util.getLangConfig().getString("messages.commands.slowchat.enabled")).replace("%player%", sender.getName()));
